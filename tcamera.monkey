...
Class TCamera Extends TEntity
	...
	Field mod_mat:Float[16]
	Field proj_mat:Float[16]
	...
	Method CameraProject:Vector(x#,y#,z#)
		...
		Local p:Vector
		p = GluProject(x,y,-z,mod_mat,proj_mat,viewport)
	...
	Method EntityInFrustum#(ent:TEntity)
		...
		If mesh
			...
			If ry<0 Then ry=-ry
			If rz<0 Then rz=-rz
			radius = Max(Max(rx,ry),rz) ''optimizing ABS()
	...
	Method Update(cam:TCamera=Null)
		...
		Local new_mat:Matrix=mat.Inverse()	
		glLoadMatrixf(new_mat.ToArray() )
		...
		'glGetIntegerv(GL_VIEWPORT,viewport) ''not working? dont really need it-- 2012
	...
	Function GluProject:Vector(x:Float, y:Float, z:Float, model:Float[], proj:Float[], viewport:Int[])
		''mod16, proj16, v4
		...
		Local mod_mat:Matrix = New Matrix
	...
	Function  GluUnProject:Vector(wx:Float, wy:Float, wz:Float, cam:TCamera)
		...
		inv_mat.Multiply4(proj_mat.Inverse4() )
