...
'' shaders are singletons. use ResetShader()
...
Class FullShader Extends TShaderGLSL
	...
	Method ResetShader()
		...
		''need to reset existing id
		If shader[0]
			For Local i:Int = 0 To 9
				shader[i].shader_id=0
			Next
		Endif
		...
		If fastbrightshader
			FastBrightShader.init_id=0
			fastbrightshader.shader_id=0
		endif
	...
	Method FreeShader()
		...
		For Local i:Int =0 To shader.Length-1
			shader[i].FreeShader()
		Next
		
		fastbrightshader.FreeShader()
...
Class MultiShader Extends TShaderGLSL
	...
	"nmLight = all_zeros.xyz; /*meaningless*/"+
	...
	Global VERTP3:String = "vertcolor = mix (basecolor, aColors, colorflag); vec4 vertpos = pMatrix * vertVec; gl_Position = vertpos; "
	
	Global VERTFOG:String = "fogBlend = 0.0;"+
	...
	Global FRAGVARS1:String = "/*material*/"+
...
#rem	
	...
	''double-sided poly lighting
	"~n"'+" if (!gl_FrontFacing) { light = light * clamp(dot(lightVec,-normal),0.0,1.0); }~n" 
	...
	Method Frag_Texture:String(i:Int)
		...
		If i>0
			...
			"finalcolor = BlendFunction(texBlend[0].x, texture, finalcolor, vertcolor); }"+
			" if(!usenormalmap && (texture.a<alphaflag)) {discard;} /* fix for tegra3 */ ~n"
	...
	Global FRAGCOLOR:String = "gl_FragColor = vec4(  mix( ((finalcolor.xyz * light.xyz +specular.xyz) + (finalcolor.xyz * ambient.xyz) ), fogColor.xyz, fogBlend), finalcolor.w );"+	
	"}"
	'Field uniform:Int[5]
	...
	Method New(ppl:Int, num_lights:Int=1, num_texs:Int=0, debug:Bool=false)
		...
		name ="FullShader PPL"+ppl+" LI"+num_lights+" TX"+num_texs
		...
		If ppl=0
			...
			VP = VP.Join([VERTVARS0,LightVars,LightingEquation0,VERTP1,VERTP2,Vert_Texture(num_texs),VERTP3,VERTFOG,Vert_Lighting0,"}~n"])
			FP = FP.Join([FRAGVARS0,FRAGVARS1,FRAGBLEND,Frag_VertexLighting0,FRAGP2,Frag_Texture(num_texs),FRAGCOLOR])
			
		Else
			''per pixel lighting

			VP = VP.Join([VERTVARS0,VERTP1,VERTP2,Vert_Texture(num_texs),VERTP3,VERTFOG,"}~n"])
			FP = FP.Join([FRAGVARS0,LightVars,Frag_LightVars,FRAGVARS1,FRAGBLEND,LightingEquation0,FRAGP2,Frag_Texture(num_texs),FRAGCOLOR])
			
		Endif
		
		'If TRender.DEBUG Then Print "ppl:"+ppl+" li:"+num_lights+" tex:"+num_texs+"~n"+VP+"~n"+FP
...
#rem
	...
	Method ResetShader()
		...
		If( CompileShader(VERTP,GL_VERTEX_SHADER) And CompileShader(FRAGP,GL_FRAGMENT_SHADER) )
			LinkShader()
			LinkVariables()
			global_uniforms = u
			init_id = shader_id
