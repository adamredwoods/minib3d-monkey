...
public class VertexDataBuffer
{
	...
	public const float INVSIZE 			= 1.0f/16.0f;
	
	public const int POS_OFFSET 		= 0;
	public const int NORMAL_OFFSET 		= 4;
	public const int COLOR_OFFSET 		= 8;
	public const int TEXCOORDS_OFFSET 	= 12;
	
	public const int ELEMENT0 			= 0;
	public const int ELEMENT1 			= 1;
	public const int ELEMENT2 			= 2;
	public const int ELEMENT3 			= 3;
	
	public float[] _buffer;
	
	public int Size()
	{
		return (_buffer.Length - 1 ) / 16;
	}
	
	public VertexDataBuffer(int s)
	{
		_buffer = new float[s*SIZE+1];
	}
		
	public static VertexDataBuffer Create(int size)
	{
		return new VertexDataBuffer(size);
	}
	
	public static VertexDataBuffer CopyVertexDataBuffer(VertexDataBuffer src, VertexDataBuffer dest)
    {
        if (src == null)
        {
            return new VertexDataBuffer(0);
        }

        Array.Copy(src._buffer, 0, dest._buffer, 0, src._buffer.Length);
        return dest;
    }
	
	public static VertexDataBuffer CopyVertexDataBuffer(VertexDataBuffer src, VertexDataBuffer dest, int begin, int bend)
	{
        ...
        if (begin == 0 && bend == 0) bend = src._buffer.Length - 1;
        if (dest._buffer.Length - 1 < bend) bend = dest._buffer.Length - 1;

        Array.Copy(src._buffer, 0, dest._buffer, begin, bend);
		return dest;
	}
	
	public void PokeCoords(int i, float x, float y, float z)
	{
		_buffer[i*SIZE + POS_OFFSET + ELEMENT0] = x;
		_buffer[i*SIZE + POS_OFFSET + ELEMENT1] = y;
		_buffer[i*SIZE + POS_OFFSET + ELEMENT2] = z;	
	}
	
	public void PokeNormals(int i, float x, float y, float z)
	{
		_buffer[i*SIZE + NORMAL_OFFSET + ELEMENT0] = x;
		_buffer[i*SIZE + NORMAL_OFFSET + ELEMENT1] = y;
		_buffer[i*SIZE + NORMAL_OFFSET + ELEMENT2] = z;	
	}
	
	public void PokeColor(int i, float r, float g, float b, float a)
	{
		_buffer[i*SIZE + COLOR_OFFSET + ELEMENT0] = r;
		_buffer[i*SIZE + COLOR_OFFSET + ELEMENT1] = g;
		_buffer[i*SIZE + COLOR_OFFSET + ELEMENT2] = b;	
		_buffer[i*SIZE + COLOR_OFFSET + ELEMENT3] = a;	
	}
	
	public void PokeTexCoords(int i, float s0, float t0, float s1, float t1)
	{
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT0] = s0;
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT1] = t0;
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT2] = s1;
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT3] = t1;
	}
	
	
	public void PokeTexCoords0(int i, float s0, float t0)
	{
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT0] = s0;
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT1] = t0;
	}
	
	public void PokeTexCoords1(int i, float s1, float t1)
	{
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT2] = s1;
		_buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT3] = t1;
	}
	...
	{
		return _buffer[i*SIZE + POS_OFFSET + ELEMENT0];
	}
	
	public float VertexY(int i)
	{
		return _buffer[i*SIZE + POS_OFFSET + ELEMENT1];
	}
	
	public float VertexZ(int i)
	{
		return _buffer[i*SIZE + POS_OFFSET + ELEMENT2];
	}
	
	public float VertexNX(int i)
	{
		return _buffer[i*SIZE + NORMAL_OFFSET + ELEMENT0];
	}
	
	public float VertexNY(int i)
	{
		return _buffer[i*SIZE + NORMAL_OFFSET + ELEMENT1];
	}
	
	public float VertexNZ(int i)
	{
		return _buffer[i*SIZE + NORMAL_OFFSET + ELEMENT2];
	}
	
	public float VertexR(int i)
	{
		return _buffer[i*SIZE + COLOR_OFFSET + ELEMENT0];
	}
	
	public float VertexG(int i)
	{
		return _buffer[i*SIZE + COLOR_OFFSET + ELEMENT1];
	}
	
	public float VertexB(int i)
	{
		return _buffer[i*SIZE + COLOR_OFFSET + ELEMENT2];
	}
	
	public float VertexA(int i)
	{
		return _buffer[i*SIZE + COLOR_OFFSET + ELEMENT3];
	}
	
	public float VertexU(int i, int coord_set)
	{
		return (coord_set == 0 )? _buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT0] : _buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT2];
	}
	
	public float VertexV(int i,int coord_set)
	{
		return (coord_set == 0 )? _buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT1] : _buffer[i*SIZE + TEXCOORDS_OFFSET + ELEMENT3];
	}
}
